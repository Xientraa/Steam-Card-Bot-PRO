...
f.getBadges = (SENDER, n, callback) => {
    Utils.getBadges(
        ...
        (ERR, DATA, CURRENTLEVEL, XPNEEDED) => {
            ...
            logcolors.true(
                "| [Refloow] |: Finished processing !level request, fun fact user is level " +
                    CURRENTLEVEL,
            );
            if (!ERR) {
                if (DATA) {
                    if (n > CURRENTLEVEL) {
                        ...
                        response =
                            "/pre ✔️ To get to level " +
                            n +
                            " you will need " +
                            (s - Math.floor(XPNEEDED / 100)) +
                            " sets. That would cost " +
                            Math.floor(
                                parseInt(
                                    ((s - Math.floor(XPNEEDED / 100)) /
                    ...
                    response =
                        "/pre ⚠️ Your level could not be retrieved. Make sure your Steam Profile is public and try again.";
                }
            } else {
                logcolors.fail(
                    "| [Steam] |: An error occurred while getting badge data: " +
                        ERR,
                );
                response =
                    "/pre ⚠️ An error occurred while loading your badges. Please try again later.";
            }
                                                        ...
                                                        f.MaxSets(
                                                            ...
                                                            (
                                                                ...
                                                                if (
                                                                    ...
                                                                    refloow.chatMessage(
                                                                        SENDER,
                                                                        "/pre ⚠️ There are currently not enough sets that you have not used in stock for this amount of keys.",
                                                                    );
                                                                }
                                                            },
                                                        ); // TO HERE
...
f.getHisSets = (
    ...
    if (!isNaN(n) && parseInt(n) > 0) {
                        ...
                        (ERR, INV, CURR) => {
                            if (ERR) {
                                ...
                                if (botKeys.length != n) {
                                    refloow.chatMessage(
                                        SENDER,
                                        "/pre ⚠️ The bot does not have enough keys.",
                                    );
                                } else {
                                                    ...
                                                    (ERR, DDATA) => {
                                                        if (!ERR) {
                                                            ...
                                                            if (amountofB > 0) {
                                                                refloow.chatMessage(
                                                                    SENDER,
                                                                    "/pre ⚠️ You do not have enough sets, (this bot only accepts total " +
                                                                        CONFIG
                                                                            .CARDS
                                                                            .MAXSETSELL +
                                                                        " sets at a time and " +
                                                                        CONFIG.MAXSTOCKSETSPERGAME +
                                                                        " per set type ). Please try again later.",
                                                                );
                                                            } else {
                                                                logcolors.info(
                                                                    "| [Debug] |: -SENDING",
        ...
        refloow.chatMessage(
            SENDER,
            "/pre ⚠️ Please enter a valid amount of keys!",
        );
    }
};
...
f.donateSets = (
    ...
    if (!isNaN(n) && parseInt(n) > 0) {
        if (n <= CONFIG.MAXSELL) {
            ...
            t.getUserDetails((ERR, ME, THEM) => {
                logcolors.true(
                    "| [Refloow] |: Processing !donatesets request...",
                );
                if (ERR) {
                    logcolors.fail(
                        "| [Debug] |: An error occurred while getting trade holds: " +
                                ...
                                Utils.getInventory(
                                    ...
                                    (ERR, DATA) => {
                                        if (!ERR) {
                                            ...
                                            logcolors.fail(
                                                "| [Steam] |: An error occurred while getting user inventory: " +
                                                    ERR,
                                            );
                                        }
                                    },
...
f.getCurrencyStocks = (manager, SENDER, refloow) => {
    ...
    manager.getInventoryContents(730, 2, true, (ERR, INV, CURR) => {
        if (ERR) {
            logcolors.fail(
                " |DEBUG|: An error occurred while getting bot inventory: " +
                    ERR,
            );
            refloow.chatMessage(
                SENDER,
        ...
        manager.getInventoryContents(440, 2, true, (ERR, INV, CURR) => {
            ...
            manager.getInventoryContents(753, 6, true, (ERR, INV, CURR) => {
                if (ERR) {
                    logcolors.fail(
                        " |DEBUG|: An error occurred while getting bots inventory: " +
                            ERR,
                    );
                    refloow.chatMessage(
                        SENDER,
                    ...
                    INV = INV.filter(
                        (ITEM) =>
                            ITEM.getTag("item_class").internal_name ==
                            "item_class_7",
                    );
                    if (INV[0] != null) gems = INV[0].amount;
                }

                refloow.chatMessage(
                    SENDER,
                    "/pre I currently have -\n" +
                        csgo +
                        " csgo keys\n" +
                        hydra +
                        " hydra keys\n" +
...
f.levelReached = (n, currency, CURRENTLEVEL, callback) => {
    ...
    for (
        level = CURRENTLEVEL;
        amountOfSets > 0 && amountOfSets > base;
        level++
    ) {
        amountOfSets -= base;
...
f.updateSteamSupplyStock = (sets) => {
    const set_counts = {};
    for (const [appid, value] of Object.entries(sets)) {
        set_counts[appid] = value.length;
    }

    return new Promise((resolve, reject) => {
        setTimeout(() => {
            fetch(
                `https://steam.supply/API/${CONFIG.STEAM_SUPPLY_API_KEY}/updateBot`,
                {
                    method: "POST",
                    body: JSON.stringify({
                        sets: set_counts,
                        selling: CONFIG.buy_enable
                            ? {
                                  tf2: CONFIG.buytf2_enable
                                      ? CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSTF2
                                      : 0,
                                  csgo: CONFIG.buycsgo_enable
                                      ? CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETS
                                      : 0,
                                  hydra: CONFIG.buyhydra_enable
                                      ? CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSHYDRA
                                      : 0,
                                  tod: 0,
                                  gem: CONFIG.buygems_enable
                                      ? CONFIG.CARDS.BUY1GEMSFORAMOUNTOFSETS
                                      : 0,
                              }
                            : {},
                        buying: CONFIG.sell_enable
                            ? {
                                  tf2: CONFIG.selltf2_enable
                                      ? CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETSTF2
                                      : 0,
                                  csgo: CONFIG.sellcsgo_enable
                                      ? CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETS
                                      : 0,
                                  hydra: CONFIG.sellhydra_enable
                                      ? CONFIG.CARDS
                                            .GIVE1KEYPERAMOUNTOFSETSHYDRA
                                      : 0,
                                  tod: 0,
                                  gem: CONFIG.sellgems_enable
                                      ? CONFIG.CARDS
    ...
    }, 5000); // Lazy Solution, wait for bot to show up as online on steam before running
              // though whenever the stock updates this will still be delayed, might fix, might not.
};
