...
fs.readFile("./app/[DB] UserData/Users.json", (ERR, DATA) => {
    if (ERR) {
        logcolors.fail(
            "| [Friends] |: An error occurred while getting Users: " + ERR,
        );
    } else {
...
Utils.getCardsInSets((ERR, DATA) => {
    if (!ERR) {
        allCards = DATA;
        logcolors.info(
            "| [Steam] |: Card data loaded. [" + Object.keys(DATA).length + "]",
        );
    } else {
        logcolors.fail(
            "| [Steam] |: An error occurred while getting card data: " + ERR,
        );
    }
});
...
refloow.on("loggedOn", (details, parental) => {
    refloow.getPersonas([refloow.steamID], (personas) => {
        logcolors.true(
            "| [Steam] |: Logged on steam as #" + refloow.steamID + "",
        );
    });
...
refloow.on("webSession", (sessionID, cookies) => {
    manager.setCookies(cookies, (ERR) => {
        if (ERR) {
            logcolors.fail(
                "| [WebSession] |: An error occurred while setting cookies.",
            );
        } else {
            logcolors.true(
                "| [WebSession] |: Websession Created And Cookies Set. \n\n",
            );
            ...
            logcolors.fail(
                "| [Refloow] |: Looks like the bot has been successfully set up on the account !",
            );
            logcolors.fail(
                "| [Refloow] |: To support the project for more updates leave an star on github repository:\n",
            );
            logcolors.fail(
                "| [GitHub] |: https://github.com/Refloow/Steam-Card-Bot-PRO",
            );
            logcolors.true(
                "| [Refloow] |: Steam-Card-Bot-ULTRA is now out, next gen software available for 27 tf2 keys at: https://discord.gg/4enDY8yhuS\n\n",
            );
            logcolors.info(
                "/////////////////////////////////////////////////////////////////////////// \n",
            );
                ...
                if (!ERR) {
                    ...
                    Utils.getSets(s, allCards, (ERR, DATA) => {
                        ...
                        if (!ERR) {
                            botSets = DATA;
                            logcolors.info(
                                "| [Inventory] |: All card sets loaded",
                            );
                            let botNSets = 0;
                            for (
                                let i = 0;
                                i < Object.keys(botSets).length;
                                i++
                            ) {
                                botNSets +=
                                    botSets[Object.keys(botSets)[i]].length;
                            }
                            totalBotSets = botNSets;
                            let playThis = CONFIG.PLAYGAMES;
                            if (
                                CONFIG.PLAYGAMES &&
                                typeof CONFIG.PLAYGAMES == "string"
                            ) {
                                playThis = parseString(playThis, totalBotSets);
                            }
                            refloow.gamesPlayed(playThis);

                            if (CONFIG.STEAM_SUPPLY_API_KEY !== "") {
                                f.updateSteamSupplyStock(botSets).catch(
                                    (err) => {
                                        logcolors.fail(err);
                                    },
                                );
                            ...
                            logcolors.true(
                                `| [Start] |: Successfully checked for offline friend requests.`,
                            );
                        } else {
                            logcolors.fail(
                                "| [Inventory] |: An error occurred while getting bot sets: " +
                                    ERR,
                            );
                            logon();
                        }
                    ...
                    logcolors.fail(
                        "| [Inventory] |: An error occurred while getting bot inventory: " +
                            ERR,
                    );
                }
            },
...
refloow.on("friendRelationship", (SENDER, REL) => {
    if (REL === 2) {
        logcolors.info(
            `| [Steam] | FRIEND |: USER ID: ${SENDER.getSteamID64()} added us on the friendlist.`,
        );
...
refloow.on("friendRelationship", function (SENDER, REL) {
    if (REL == 0) {
        logcolors.fail(
            `| [Steam] | FRIEND |: USER ID: ${SENDER.getSteamID64()} is no longer on our friendlist.`,
        );
    }
});
...
refloow.on("friendRelationship", function (SENDER, REL) {
    if (REL == 3) {
        logcolors.true(
            `| [Steam] | FRIEND |: USER ID: ${SENDER.getSteamID64()} is now on our friendlist.`,
        );
    }
});
    ...
    setInterval(
        () => {
            for (let i = 0; i < Object.keys(users).length; i++) {
                if (
                    users[Object.keys(users)[i]].idleforhours >=
                    CONFIG.MAXHOURSADDED
                ) {
                    if (method.SendingMessageToRemovedInactive()) {
                        refloow.chatMessage(
                            Object.keys(users)[i],
                            CONFIG.REMOVEDINACTIVE,
                        );
                    }
                    refloow.removeFriend(Object.keys(users)[i]);
                    delete users[Object.keys(users)[i]];
                    fs.writeFile(
                        "./app/[DB] UserData/Users.json",
                        JSON.stringify(users),
                        (ERR) => {
                            if (ERR) {
                                logcolors.error(
                                    "| |UserData| |: An error occurred while writing UserData file: " +
                                        ERR,
                                );
                            }
                        },
...
if (method.DecliningRandomGroupInvites()) {
    refloow.on("groupRelationship", function (sid, REL) {
        if (REL == SteamUser.EClanRelationship.Invited) {
            logcolors.info(
                "| [Steam] |: We were asked to join steam group #" + sid,
            ); //cyan
                ...
                for (let j = 0; j < CONFIG.ADMINS.length; j++) {
                    if (method.SpamAdminNotification()) {
                        refloow.chatMessage(
                            ...
                            "User #" +
                                Object.keys(userMsgs)[i] +
                                " has been removed for spamming. To block him use !block [STEAMID64] (Command will work after adding admin commands to system.)",
                        );
                    }
                }
            }
...
refloow.on("friendMessage", (SENDER, MSG) => {
    if (method.RefloowChat()) {
        logcolors.new(
            "| [RefloowChat] |: " + SENDER.getSteamID64() + " |: " + MSG,
        );
    }
    ...
    if (method.ChatLogsForEachUserEnabled()) {
        fs.writeFile(
            "./app/[DB] ChatLogs/UserLogs/" +
                ...
                ".json",
            JSON.stringify({ logs: userLogs[SENDER.getSteamID64()] }),
            (ERR) => {
                if (ERR) {
                    logcolors.fail(
                        "| [Users] |: An error occurred while writing UserLogs file: " +
                            ERR,
                    );
                }
            },
    ...
    if (method.DailyChatLogsEnabled()) {
        ...
        fs.writeFile(
            ...
            (ERR) => {
                if (ERR) {
                    logcolors.fail(
                        "| [Users] |: An error occurred while writing FullLogs file: " +
                            ERR,
                    );
                }
            },
    ...
    if (Object.keys(users).indexOf(SENDER.getSteamID64()) < 0) {
        ...
        fs.writeFile(
            ...
            (ERR) => {
                if (ERR) {
                    logcolors.fail(
                        "| [Users] |: An error occurred while writing UserData file: " +
                            ERR,
                    );
                }
            },
    ...
    // !PRICE, !PRICES, !RATE commands
        ...
        refloow.chatMessage(
            SENDER,
            `/code The current prices are:\n` +
                "________________________\n" +
                "|____ME___|_____YOU_____|\n" +
                "| " +
                CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETS +
                ` sets ` +
                "|" +
                ` 1 CSGO key ` +
                " |\n" +
                "| " +
                CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSHYDRA +
                ` sets ` +
                "|" +
                ` 1 Hydra key ` +
                "|\n" +
                "| " +
                CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSTF2 +
                ` sets ` +
                "|" +
                ` 1 TF 2 key ` +
                " |\n" +
                ...
                `01 set ⠀` +
                "| " +
                CONFIG.CARDS.BUY1GEMSFORAMOUNTOFSETS +
                ` gems ` +
                "   |\n\n" +
                "_________________________\n" +
                "|___YOU___|_____ME______|\n" +
                "| " +
                CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETS +
                ` sets ` +
                "|" +
                ` 1 CSGO key ` +
                " |\n" +
                "| " +
                CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETSHYDRA +
                ` sets ` +
                "|" +
                ` 1 Hydra key ` +
                "|\n" +
                "| " +
                CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETSTF2 +
                ` sets ` +
                "|" +
                ` 1 TF 2 key ` +
                " |\n" +
                ...
                `01 set ⠀` +
                "| " +
                CONFIG.CARDS.GIVE1GEMSFORAMOUNTOFSETSGEMS +
                ` gems ` +
                "   |\n",
        );
    }
    ...
    // This will log when this commands were executed
        ...
        if (method.LogsCalledCommandsEnabled()) {
            logcolors.true(
                "| [Refloow] |: Displaying prices, !PRICE command called by " +
                    SENDER.getSteamID64(),
            );
        }
        ...
        if (method.LogsCalledCommandsEnabled()) {
            logcolors.true(
                "| [Refloow] |: Displaying prices, !PRICES command called by " +
                    SENDER.getSteamID64(),
            );
        }
        ...
        if (method.LogsCalledCommandsEnabled()) {
            logcolors.true(
                "| [Refloow] |: Displaying prices, !RATE command called by " +
                    SENDER.getSteamID64(),
            );
        }
        ...
        if (method.LogsCalledCommandsEnabled()) {
            logcolors.true(
                "| [Help] |: Displaying all commands, !help command called by " +
                    SENDER.getSteamID64(),
            );
        }
        ...
        if (method.LogsCalledCommandsEnabled()) {
            logcolors.true(
                "| [Help] |: Displaying all commands, !commands command called by " +
                    SENDER.getSteamID64(),
            );
        }
    }
    ...
    // !Stats command
        ...
        logcolors.info(
            "| [Refloow] |: !stats command called by " + SENDER.getSteamID64(),
        );
    ...
    // !Level command
        ...
        if (!isNaN(n) && parseInt(n) > 0) {
            if (n <= CONFIG.MAXLEVEL) {
                ...
                refloow.chatMessage(
                    SENDER,
                    "/pre ⚠️ Please try a lower level.",
                );
            }
        } else {
            refloow.chatMessage(
                SENDER,
                "/pre ⚠️ Please provide a valid level.",
            );
        }
        ...
        if (Object.keys(botSets).length == 0) {
            refloow.chatMessage(
                SENDER,
                "/pre Bot doesn't have any sets at the moment, please check later.",
            );
            logcolors.true(
                "| [Refloow] |: Finished processing !check request bot doenst have any sets in the inventory",
            );
        }
        if (Object.keys(botSets).length > 0) {
            ...
            Utils.getBadges(SENDER.getSteamID64(), (ERR, DATA) => {
                ...
                if (!ERR) {
                    f.MaxSets(DATA, botSets, (hisMaxSets, botNSets) => {
                        ...
                        logcolors.true(
                            "| [Refloow] |: Finished processing of !check request",
                        );
                        logcolors.summary(
                            "| [Summary] |: Bot has " +
                                hisMaxSets +
                        ...
                        refloow.chatMessage(
                            SENDER,
                            "/pre There are currently " +
                                hisMaxSets +
                                "/" +
                                botNSets +
                                " sets available which you have not fully crafted yet.Buying all of them will cost you -\n" +
                                "- " +
                                parseInt(
                                    hisMaxSets /
                                        CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSTF2,
                    ...
                    refloow.chatMessage(
                        SENDER,
                        "/pre ⚠️ An error occurred while getting your badges. Please try again.",
                    );
                    logcolors.fail(
                        SENDER,
                        "| [Steam] |: An error occurred while getting badges: " +
                            ERR,
                    );
                }
            });
            ...
            if (method.LogsCalledCommandsEnabled()) {
                ...
                if (!ERR) {
                    f.MaxSets(DATA, botSets, (hisMaxSets, botNSets) => {
                        ...
                        refloow.chatMessage(
                            SENDER,
                            "/code There are currently sets from " +
                                Object.keys(botSets).length +
                                " different games, of which you have not crafted " +
                                hisMaxSets +
                                ". This would cost " +
                                parseInt(
                                    (hisMaxSets /
                    ...
                    logcolors.fail(
                        SENDER,
                        "| [Steam] |: An error occurred while loading badges: " +
                            ERR,
                    );
                }
            } else {
                refloow.chatMessage(
                    SENDER,
                    "/pre ⚠️ An error occurred, command has been disabled by admin",
                );
            }
        });
    }
    ...
    // Reminder command
        ...
        if (type == "ON") {
            fs.readFile("./app/[DB] UserData/reminder.json", (ERR, DATA) => {
                if (ERR)
                    logcolors.fail(
                        "| [Debug] | An error occurred while reading file " +
                            ERR,
                    );
                else {
                    try {
                        ...
                        fs.writeFile(
                            "./app/[DB] UserData/reminder.json",
                            jsonString,
                            (ERR) => {
                                if (ERR)
                                    logcolors.fail(
                                        "| [Debug] | An error occurred while writing to file " +
                                            ERR,
                                    );
                                else
                                    refloow.chatMessage(
                                        SENDER.getSteamID64(),
                                        "/pre Successfully added your steamID to list!",
                                    );
                            },
                        );
                    } catch (err) {
                        logcolors.fail(
                            "| [Debug] | An error occurred while parsing string " +
                                err,
                        );
                    }
                }
                ...
                if (ERR)
                    ...
                    try {
                        ...
                        if (bool) {
                            ...
                            fs.writeFile(
                                ...
                                (ERR) => {
                                    if (ERR)
                                        ...
                                        refloow.chatMessage(
                                            SENDER.getSteamID64(),
                                            "/pre Successfully removed your steamID from list!",
                                        );
                                },
                            );
                        } else
                            refloow.chatMessage(
                                SENDER.getSteamID64(),
                                "/pre Could not find your steamID in list",
                            );
                    } catch (err) {
                        logcolors.fail(
                            "| [Debug] | An error occurred while parsing string " +
            ...
            refloow.chatMessage(
                SENDER.getSteamID64(),
                "/pre Please select valid option!",
            );
        }
    }
    ...
    // !SELLCHECK COMMAND
        ...
        Utils.getInventory(SENDER.getSteamID64(), community, (ERR, DATA) => {
            if (method.LogsCalledCommandsEnabled()) {
                logcolors.true(
                    "| [Refloow] |: Processing !checksell requested by " +
                        SENDER.getSteamID64(),
                );
            }
                    ...
                    if (!ERR) {
                        ...
                        for (let i = 0; i < Object.keys(DATA).length; i++) {
                            if (botSets[Object.keys(DATA)[i]]) {
                                if (
                                    botSets[Object.keys(DATA)[i]].length >=
                                    CONFIG.MAXSTOCKSETSPERGAME
                                ) {
                                    continue;
                                }
                                var total =
                                    botSets[Object.keys(DATA)[i]].length +
                                    DATA[Object.keys(DATA)[i]].length;
                        ...
                        refloow.chatMessage(
                            SENDER,
                            "/pre You currently have " +
                                botNSets +
                                " sets available which I can buy. I will pay you:\n" +
                                parseInt(
                                    botNSets /
                                        CONFIG.CARDS.GIVE1KEYPERAMOUNTOFSETSTF2,
                                ) +
                        ...
                        logcolors.true(
                            "| [Refloow] |: User has " +
                                botNSets +
                                " sets available to sell",
                        );

                        f.getCurrencyStocks(manager, SENDER, refloow);
                    } else {
                        logcolors.fail(
                            "| [Inventory] |: An error occurred while getting user sets: " +
                                ERR,
                        );
                        refloow.chatMessage(
                            "An error occurred while getting your inventory. Please try again later",
                        );
                    }
                });
            } else {
                logcolors.fail(
                    "| [Inventory] |: An error occurred while getting inventory: " +
                        ERR,
                );
            }
        });
    ...
    // !Check command
        ...
        if (!isNaN(n) && parseInt(n) > 0) {
            if (method.LogsCalledCommandsEnabled()) {
                logcolors.true(
                    "| [Refloow] |: Processing !check requested by " +
                        SENDER.getSteamID64(),
                );
                                        ...
                                        f.levelReached(
                                            ...
                                            (hydralevel) => {
                                                refloow.chatMessage(
                                                    ...
                                                    "/code ✔️ With " +
                                                        n +
                                                        " CS:GO keys you can get " +
                                                        n *
                                                            CONFIG.CARDS
                                                                .BUY1KEYFORAMOUNTOFSETS +
                                                        " sets and reach level " +
                                                        cslevel +
                                                        "\n✔️ With " +
                                                        n +
                                                        " TF2 keys you can get " +
                                                        n *
                                                            CONFIG.CARDS
                                                                .BUY1KEYFORAMOUNTOFSETSTF2 +
                                                        " sets and reach level " +
                                                        tflevel +
                                                        "\n✔️ With " +
                                                        n +
                                                        " Hydra keys you can get " +
                                                        n *
                                                            CONFIG.CARDS
                                                                .BUY1KEYFORAMOUNTOFSETSHYDRA +
                                                        " sets and reach level " +
            ...
            refloow.chatMessage(
                SENDER,
                "/pre ⚠️ Please provide a valid amount of keys.",
            );
            logcolors.true(
                "| [Refloow] |: Command was sent without check number indicator",
            );
        }
    }
    ...
    // !Donatesets command
        ...
        if (botSets) {
            ...
            f.donateSets(
                ...
                (callback) => {
                    logcolors.true(
                        "| [Refloow] |: Finished processing !donateSets request",
                    );
                },
            );
        } else {
            refloow.chatMessage(
                SENDER,
                "/pre ⚠️ Please try again later. (Steam is down or command is disabled by Admin)",
            );
        }
    }
    ...
    // HERE IS SECTION WITH SIMPLE BUY COMMANDS USING MAIN BOT CURRENCY SET IN CONFIG (!BUY, !BUYANY, !BUYONE)
        ...
        if (botSets) {
            let n = MSG.toUpperCase().replace("!BUYANY ", ""),
                amountofsets =
                    parseInt(n) * CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETS;
            if (method.UserBuyingAny()) {
                if (!isNaN(n) && parseInt(n) > 0) {
                    if (n <= CONFIG.MAXBUY) {
                        ...
                        t.getUserDetails((ERR, ME, THEM) => {
                            if (ERR) {
                                logcolors.fail(
                                    "| [Debug] |: An error occurred while getting trade holds: " +
                                        ERR,
                                );
                                refloow.chatMessage(
                                    SENDER,
                                    "/pre ⚠️ An error occurred while getting your trade holds. Please try again",
                                );
                            } else if (
                                ME.escrowDays == 0 &&
                                THEM.escrowDays == 0
                                ...
                                refloow.chatMessage(
                                    SENDER,
                                    "/me ⚠️ Processing your request.",
                                );
                                logcolors.true(
                                    "| [Refloow] |: Processing of !buyany request",
                                );
                                manager.getUserInventoryContents(
                                    ...
                                    (ERR, INV, CURR) => {
                                        if (ERR) {
                                            ...
                                            refloow.chatMessage(
                                                SENDER,
                                                "/pre ⚠️ An error occurred while loading your inventory. Please try later",
                                            );
                                        } else {
                                            ...
                                            if (theirKeys.length != n) {
                                                refloow.chatMessage(
                                                    SENDER,
                                                    "/pre ⚠️ You do not have enough keys.",
                                                );
                                            } else {
                                                sortSetsByAmount(
                                                    botSets,
                                                                ...
                                                                for (
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            logcolors.info(
                                                                                "| [Debug] |: loop #2 CONTINUE: ITEM ADD",
                                                                            );
                                                                            amountofB--;
                                                                            if (
                                                                                !setsSent[
                                            ...
                                            if (amountofB > 0) {
                                                refloow.chatMessage(
                                                    SENDER,
                                                    "/pre ⚠️ There are not enough sets. Please try again later.",
                                                );
                                            } else {
                                                logcolors.info(
                                                    "| [Debug] |: -SENDING",
                                                ...
                                                t.data(
                                                    "amountofsets",
                                                    amountofsets.toString(),
                                                );
                                                t.data("amountofkeys", n);
                                                t.data(
                                                    "index",
                                                    setsThatShouldntBeSent.length,
                                                );
                                                setsThatShouldntBeSent.push(
                                                    t.itemsToGive,
                                                );
                                                t.send((ERR, STATUS) => {
                                                    if (ERR) {
                                                        refloow.chatMessage(
                                                            SENDER,
                                                            "/pre ⚠️ An error occurred while sending your trade. Steam Trades could be down. Please try again later.",
                                                        );
                                                        logcolors.fail(
                                                            "| [Steam] |: An error occurred while sending trade: " +
                                                                ERR,
                                                        );
                                                    } else {
                                                        refloow.chatMessage(
                                                            SENDER,
                                                            "/me ✔️ Trade Sent! Confirming it...",
                                                        );
                                                        logcolors.info(
                                ...
                                refloow.chatMessage(
                                    SENDER,
                                    "/pre ⚠️ Please make sure you don't have a trade hold!",
                                );
                            }
                        });
                    } else {
                        refloow.chatMessage(
                            SENDER,
                            "/pre ⚠️ Please try a lower amount of keys.",
                        );
                    }
                } else {
                    refloow.chatMessage(
                    ...
                    if (n <= CONFIG.MAXBUY) {
                        ...
                        t.getUserDetails((ERR, ME, THEM) => {
                            if (ERR) {
                                ...
                                let theirKeys = [];
                                logcolors.true(
                                    "| [Refloow] |: Processing of !buyone request",
                                );
                                refloow.chatMessage(
                                    SENDER,
                                    "/me ⚠️ Processing your request.",
                                    ...
                                    (ERR, INV, CURR) => {
                                        if (ERR) {
                                            ...
                                            if (!ERR) {
                                                            ...
                                                            if (!ERR) {
                                                                ...
                                                                if (!ERR) {
                                                                    ...
                                                                    if (DATA) {
                                                                        ...
                                                                        refloow.chatMessage(
                                                                            SENDER.getSteamID64(),
                                                                            "/pre Your badges are empty, sending an offer without checking badges.",
                                                                        );
                                                                    }
                                                                    console.log(
                                                                        DATA,
                                                                    ...
                                                                    if (
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                bLoop: for (
                                                                                                    ...
                                                                                                    if (
                                                                                                        ...
                                                                                                        logcolors.info(
                                                                                                            "| [Debug] |: loop #1 CONTINUE: ITEM ADD",
                                                                                                        );
                                                                                                        hisMaxSets--;
                                                                                                        continue firstLoop;
                                                                                                    } else {
                                                                        ...
                                                                        refloow.chatMessage(
                                                                            SENDER,
                                                                            "/pre ⚠️ There are currently not enough sets that you have not used in stock for this amount of keys. Please try again later. If you want the bot to ignore your current badges use !buyany.",
                                                                        );
                                                                    }
                                                                } else {
                                                                    logcolors.fail(
                                                ...
                                                refloow.chatMessage(
                                                    SENDER,
                                                    "/pre ⚠️ An error occurred while loading your inventory, please make sure it's set to public.",
                                                );
                                            }
                                        }
                                    },
                        ...
                        if (n <= CONFIG.MAXBUYREF) {
                            ...
                            t.getUserDetails((ERR, ME, THEM) => {
                                if (ERR) {
                                    ...
                                    let theirRef = [];
                                    logcolors.true(
                                        "| [Refloow] |: Processing of !buyref request",
                                    );
                                    refloow.chatMessage(
                                        SENDER,
                                        "/me Processing your request.",
                                    );
                                    manager.getUserInventoryContents(
                                        SENDER.getSteamID64(),
                                        440,
                                            ...
                                            if (ERR) {
                                                ...
                                                if (!ERR) {
                                                    ...
                                                    if (theirRef.length != n) {
                                                        refloow.chatMessage(
                                                            SENDER,
                                                            "/pre ⚠️ You do not have enough Refined Metal.",
                                                        );
                                                    } else {
                                                        Utils.getBadges(
                                                            SENDER.getSteamID64(),
                                                            (ERR, DATA) => {
                                                                if (!ERR) {
                                                                    ...
                                                                    if (!ERR) {
                                                                        let b =
                                                                            {}; // List with badges that CAN still be crafted
                                                                        if (
                                                                            DATA
                                                                        ) {
                                                                            for (
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            refloow.chatMessage(
                                                                                SENDER,
                                                                                "/pre There are currently not enough sets that you have not used in stock for this amount of Refined Metal.",
                                                                            );
                                                                        }
                                                                        // TO HERE
                                                                    } else {
                            ...
                            refloow.chatMessage(
                                SENDER,
                                "/pre ⚠️ Please try a lower amount of Refined Metal.",
                            );
                        }
                    } else {
                        refloow.chatMessage(
                            SENDER,
                            "/pre ⚠️ Please provide a valid amount of Refined Metal.",
                        );
                    }
                } else {
                    refloow.chatMessage(
                        SENDER,
                        "/pre ⚠️ Each set costs 2 ref. Try again using an even amount of Refined Metal.",
                    );
                }
            } else {
                refloow.chatMessage(
        ...
        if (botSets) {
            let n = MSG.toUpperCase().replace("!BUYHYDRA ", ""),
                amountofsets =
                    parseInt(n) * CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSHYDRA;
            if (method.UserBuyingWithHydra()) {
                ...
                f.getHisKeys(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of buyhydra command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            ...
            if (method.UserBuyingWithCSGO()) {
                ...
                f.getHisKeys(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of buycs command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            let n = MSG.toUpperCase().replace("!BUYTF ", ""),
                amountofsets =
                    parseInt(n) * CONFIG.CARDS.BUY1KEYFORAMOUNTOFSETSTF2;
            if (method.UserBuyingWithTF2()) {
                ...
                f.getHisKeys(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of buytf command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            let n = MSG.toUpperCase().replace("!BUYGEMS ", ""),
                amountofgems =
                    parseInt(n) * CONFIG.CARDS.BUY1GEMSFORAMOUNTOFSETS;
            if (method.UserBuyingWithGems()) {
                if (!isNaN(n) && parseInt(n) > 0) {
                    if (n <= CONFIG.MAXBUY) {
                        let t = manager.createOffer(SENDER.getSteamID64());
                        t.getUserDetails((ERR, ME, THEM) => {
                            if (ERR) {
                                ...
                                let theirGems = [];
                                logcolors.true(
                                    "| [Refloow] |: Processing of !buyGems request",
                                );
                                refloow.chatMessage(
                                    SENDER,
                                    "/me Processing your request.",
                                        ...
                                        if (ERR) {
                                            ...
                                            if (!ERR) {
                                                ...
                                                if (
                                                    ...
                                                    logcolors.info(
                                                        "| [Debug] |: User does not have any gems at all.",
                                                    );
                                                    refloow.chatMessage(
                                                        SENDER.getSteamID64(),
                                                        "/pre ⚠️ Looks like your inventory doesn't have any gems at all. Please try after you have them.",
                                                    );
                                                } else {
                                                    if (
                                                        INV[0].amount <
                                                        ...
                                                        logcolors.fail(
                                                            "| [Debug] |: User does not have enough gems.",
                                                        );
                                                        refloow.chatMessage(
                                                            SENDER,
                                                            "/pre ⚠️ Please make sure you have enough unpacked gems.",
                                                        );
                                                    } else {
                                                        INV[0].amount =
                                                            amountofgems.toString();
                                                                ...
                                                                if (!ERR) {
                                                                    ...
                                                                    if (!ERR) {
                                                                        ...
                                                                        if (
                                                                                        ...
                                                                                        t.send(
                                                                                            (
                                                                                                ...
                                                                                                if (
                                                                                                    ...
                                                                                                    refloow.chatMessage(
                                                                                                        SENDER,
                                                                                                        "⚠️ An error occurred while sending your trade. Steam Trades could be down. Please try again later.",
                                                                                                    );
                                                                                                    logcolors.fail(
                                                                                                        "| [Steam] |: An error occurred while sending trade: " +
                                                                                                            ERR,
                                                                            ...
                                                                            refloow.chatMessage(
                                                                                SENDER,
                                                                                "/pre ⚠️ There are currently not enough sets that you have not used in stock for this amount of Gems.",
                                                                            );
                                                                        }
                                                                        // TO HERE
                                                                    } else {
                        ...
                        refloow.chatMessage(
                            SENDER,
                            "/pre ⚠️ Please try a lower amount of Gems.",
                        );
                    }
                } else {
                    refloow.chatMessage(
                        SENDER,
                        "/pre ⚠️ Please provide a valid amount of Sets.",
                    );
                }
            } else {
                refloow.chatMessage(
        ...
        if (botSets) {
            ...
            if (method.UserBuying()) {
                ...
                f.getHisKeys(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of buy command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            ...
            if (method.UserSellingWithHydra()) {
                ...
                f.getHisSets(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of sellhydra command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            ...
            if (method.UserSellingWithCSGO()) {
                ...
                f.getHisSets(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of sellcs command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            ...
            if (method.UserSellingWithTF2()) {
                ...
                f.getHisSets(
                    ...
                    (callback) => {
                        logcolors.true(
                            "| [Refloow] |: Finished processing of selltf command!",
                        );
                    },
                );
            } else {
        ...
        if (botSets) {
            ...
            if (method.UserSellingWithGems) {
                if (!isNaN(n) && parseInt(n) > 0) {
                    if (n <= CONFIG.MAXSELL) {
                        logcolors.true(
                            "| [Refloow] |: Processing of !sellgems request",
                        );
                        refloow.chatMessage(
                            SENDER,
                            "/me ✔️ Processing your request.",
                            ...
                            if (ERR) {
                                ...
                                manager.getUserInventoryContents(
                                    ...
                                    (ERR, INV, CURR) => {
                                        if (ERR) {
                                            logcolors.fail(
                                                "| [Steam] |: An error occurred while getting bot inventory: " +
                                                    ERR,
                                            );
                                            refloow.chatMessage(
                                                SENDER,
                                                "/pre ⚠️ An error occurred while loading the bot's inventory. Please try again.",
                                            );
                                        } else {
                                            INV = INV.filter(
                                                (ITEM) =>
                                                    ITEM.getTag("item_class")
                                                        .internal_name ==
                                                    "item_class_7",
                                            );
                                            ...
                                            if (INV[0] == null) {
                                                logcolors.info(
                                                    "| [Debug] |: We do not have enough gems for this trade",
                                                );
                                                refloow.chatMessage(
                                                    SENDER,
                                                    "/pre ⚠️ The bot does not have enough gems for this trade.",
                                                );
                                            } else {
                                                if (
                                                    INV[0].amount >=
                                                    ...
                                                    Utils.getInventory(
                                                        ...
                                                        (ERR, DATA) => {
                                                            if (!ERR) {
                                                                    ...
                                                                    (
                                                                        ...
                                                                        if (
                                                                                ...
                                                                                (
                                                                                    ...
                                                                                    firsttLoop: for (
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            logcolors.info(
                                                                                                "| [Debug] |: loop #2 CONTINUE: RETURN 2",
                                                                                            );
                                                                                            continue firsttLoop;
                                                                                        }
                                                                                    }
                                                                            ...
                                                                            if (
                                                                                ...
                                                                                refloow.chatMessage(
                                                                                    SENDER,
                                                                                    "/pre ⚠️ You do not have enough sets, (this bot only keeps total " +
                                                                                        CONFIG.MAXSTOCKSETSPERGAME +
                                                                                        " games in stock at a time ). Please try again later.",
                                                                                );
                                                                            } else {
                                                                                logcolors.info(
                                                                                    "| [Debug] |: -SENDING",
                                                                            ...
                                                                            logcolors.fail(
                                                                                "| [Inventory] |: An error occurred while getting bot gems: " +
                                                                                    ERR,
                                                                            );
                                                                        }
                                                                    },
                                                                ...
                                                                logcolors.fail(
                                                                    "| [Inventory] |: An error occurred while loading user inventory: " +
                                                                        ERR,
                                                                );
                                                            }
                                                        },
                                                    ...
                                                    refloow.chatMessage(
                                                        SENDER,
                                                        "/pre Bot does not have enough gems.Please try again later",
                                                    );
                                                }
                                            }
                                        }
                        ...
                        refloow.chatMessage(
                            SENDER,
                            "/pre ⚠️ Please try a lower amount of sets.",
                        );
                    }
                } else {
                    refloow.chatMessage(
                        SENDER,
                        "/pre ⚠️ Please enter a valid amount of sets!",
                    );
                }
            } else {
                refloow.chatMessage(
            ...
            if (method.UserSell()) {
                ...
                if (method.UserSellingWithCSGO()) {
                    ...
                    f.getHisSets(
                        ...
                        (callback) => {
                            logcolors.true(
                                "| [Refloow] |: Finished processing of sellcsgo command!",
                            );
                        },
                    );
                } else {
        ...
        CONFIG.ADMINS.indexOf(SENDER.getSteamID64()) >= 0 ||
        CONFIG.ADMINS.indexOf(parseInt(SENDER.getSteamID64())) >= 0
    ) {
        // Admin commands.
        if (MSG.toUpperCase().indexOf("!BLOCK") >= 0) {
            ...
            if (SID64REGEX.test(n)) {
                ...
                refloow.chatMessage(
                    SENDER,
                    "/pre ⚠️ Please provide a valid SteamID64",
                );
            }
                        ...
                        if (!ERR) {
                            ...
                            if (DATA) {
                                ...
                                refloow.chatMessage(
                                    SENDER.getSteamID64(),
                                    n +
                                        "'s badges are empty, sending an offer without checking badges.",
                                );
                            }
                            // console.log(b);
                            // TODO: COUNT AMOUNT OF SETS BOT CAN GIVE HIM
                            ...
                            for (let i = 0; i < Object.keys(b).length; i++) {
                                if (
                                    botSets[Object.keys(b)[i]] &&
                                    botSets[Object.keys(b)[i]].length >=
                                        5 - b[Object.keys(b)[i]].length
                                ) {
                                    hisMaxSets +=
                                        5 - b[Object.keys(b)[i]].length;
                                }
                            ...
                            for (
                                ...
                                if (
                                    Object.keys(b).indexOf(
                                        Object.keys(botSets)[i],
                                    ) < 0
                                ) {
                                    if (
                                        botSets[Object.keys(botSets)[i]]
                                            .length >= 5
                                    ) {
                                        hisMaxSets += 5;
                                    } else {
                            ...
                            refloow.chatMessage(
                                ...
                                "/pre There are currently " +
                                    ...
                                    "/" +
                                    botNSets +
                                    " sets available which " +
                                    n +
                                    " has not fully crafted yet. Buying all of them will cost " +
                                    parseInt(
                                        (hisMaxSets /
                                            CONFIG.CARDS
                                                .BUY1KEYFORAMOUNTOFSETS) *
                            ...
                            refloow.chatMessage(
                                SENDER,
                                "/pre ⚠️ An error occurred while getting " +
                                    n +
                                    "'s badges. Please try again.",
                            );
                            logcolors.fail(
                                SENDER,
                                "| [Steam] |: An error occurred while getting badges: " +
        ...
        // Withdraw command
            ...
            manager.getInventoryContents(440, 2, true, (ERR, INV, CURR) => {
                if (ERR) {
                    refloow.chatMessage(
                        SENDER,
                        "/pre ⚠️ An error occurred while loading the bot's inventory.",
                    );
                    logcolors.fail(
                        "| [Inventory] |: An error occurred while getting inventory: " +
                            ERR,
                    ...
                    refloow.chatMessage(
                        SENDER,
                        "/me ✔️ Bot's inventory loaded.",
                    );
                    logcolors.info("| [Inventory] |: Bot's inventory loaded.");

                    for (let i = 0; i < INV.length; i++) {
                        if (
                            CONFIG.TF2KEY.indexOf(INV[i].market_hash_name) >= 0
                        ) {
                            t.addMyItem(INV[i]);
        ...
        // Admin command RESTART
            ...
            refloow.logOff();
            logcolors.info(
                "| [Refloow] |: Logged off... Waiting 15 sec for exceed limit",
            );
            setTimeout(() => {
                logon();
            }, 15000);
        ...
        // Admin command !EXIT (Stops bot totally from functioning by logging it off of an account)
            ...
            logcolors.true(
                "| [Refloow] |: Received forcestop command, exiting...",
            );
            refloow.logOff();
        ...
        // Admin command !pause (Pauses bot for 2 hours and then restart)
            ...
            refloow.logOff();
            logcolors.info(
                "| [Refloow] |: Bot does not respond on any requests in next 5 min after that it will restart.",
            );
            setTimeout(() => {
                logon();
            ...
            logcolors.true(
                "| [Refloow] |: Pausing the bot for next 10 min ...",
            );
            refloow.logOff();
            logcolors.info(
                "| [Refloow] |: Bot does not respond on any requests in next 10 min after that it will restart.",
            );
            setTimeout(() => {
                logon();
            ...
            logcolors.true(
                "| [Refloow] |: Pausing the bot for next 1 hours ...",
            );
            refloow.logOff();
            logcolors.info(
                "| [Refloow] |: Bot does not respond on any requests in next 1 hour after that it will restart.",
            );
            setTimeout(() => {
                logon();
            ...
            logcolors.true(
                "| [Refloow] |: Pausing the bot for next 2 hours ...",
            );
            refloow.logOff();
            logcolors.info(
                "| [Refloow] |: Bot does not respond on any requests in next 2 hours after that it will restart.",
            );
            setTimeout(() => {
                logon();
        ...
        // Broadcast reminder command - broadcasts the reminder to everyone when called
            ...
            refloow.chatMessage(
                SENDER,
                "/pre ⚠️ Command not recognized. Type !commands or !admincommands to see all the commands.",
            );
        }
    }
    ...
    // This is displayed to normal users when command is not recognized.
        ...
        refloow.chatMessage(
            SENDER,
            "/pre ⚠️ Command not recognized. Type !commands or !help to see all the commands.",
        );
    }
});
...
manager.on("sentOfferChanged", (OFFER, OLDSTATE) => {
    if (OFFER.state == 2) {
        refloow.chatMessage(
            OFFER.partner,
            "Trade confirmed! Click here to accept it: https://www.steamcommunity.com/tradeoffer/" +
                OFFER.id,
        );
            ...
            (ERR, DATA) => {
                if (!ERR) {
                    ...
                    Utils.getSets(s, allCards, (ERR, DATA) => {
                        ...
                        if (CONFIG.STEAM_SUPPLY_API_KEY !== "") {
                            f.updateSteamSupplyStock(botSets).catch((err) => {
                                logcolors.fail(err);
                            });
                        }
                    });
        ...
        fs.writeFile(
            ...
            (ERR) => {
                if (ERR) {
                    logcolors.fail(
                        "| [Steam] |: An error occurred while writing trade file: " +
                            ERR,
                    );
                }
            },
        ...
        community.getSteamUser(OFFER.partner, (ERR, USER) => {
            if (ERR) {
                logcolors.fail(
                    "| [Steam] |: An error occurred while getting user profile: " +
                        ERR,
                );
                refloow.chatMessage(
                    OFFER.partner,
                    "An error occurred while getting your profile (to comment).",
                );
            } else {
                if (method.TradeCommentEnabled()) {
                    USER.comment(CONFIG.COMMENTAFTERTRADE, (ERR) => {
                        if (ERR) {
                            logcolors.fail(
                                "| [Steam] |: An error occurred while commenting on user profile: " +
                                    ERR,
                            );
                            refloow.chatMessage(
                                OFFER.partner,
        ...
        refloow.chatMessage(
            OFFER.partner,
            "Hey, you did not accept the offer. Please try again if you wish to receive sets!",
        );
    }
});
...
manager.on("newOffer", (OFFER) => {
    if (
        CONFIG.ADMINS.indexOf(OFFER.partner.getSteamID64()) >= 0 ||
        CONFIG.ADMINS.indexOf(parseInt(OFFER.partner.getSteamID64())) >= 0
    ) {
        OFFER.getUserDetails((ERR, ME, THEM) => {
            if (ERR) {
                ...
                refloow.chatMessage(
                    OFFER.partner,
                    "An error occurred while getting your trade holds. Please try again",
                );
            } else if (ME.escrowDays == 0 && THEM.escrowDays == 0) {
                OFFER.accept((ERR) => {
                    if (ERR) {
                        logcolors.fail(
                            "| [Steam] |: An error occurred while declining trade: " +
                                ERR,
                        );
                    } else {
                        refloow.chatMessage(OFFER.partner, "Offer accepted!");
                        logcolors.true(
                            "| [Offers] |: New offer has been accepted !",
                        );
                    }
                });
            } else {
...
community.on("newConfirmation", (CONF) => {
    ...
    community.acceptConfirmationForObject(
        ...
        (ERR) => {
            if (ERR) {
                logcolors.fail(
                    "| [Steam] |: An error occurred while accepting confirmation: " +
                        ERR,
                );
            } else {
